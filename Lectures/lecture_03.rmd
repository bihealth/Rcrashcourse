...
## Exercise 3.0: reading your first file
...
(remember, "directory" is just an older word for "folder")

 * absolute path start at root directory, e.g.  
 `C:/Users/foobar/CUBI/teaching/202403_RCrashCourse/lecture01.R`
 * Note the slash (`/`) instead of backslash (`\`). 
 * Where am I? - `getwd()`
 * relative paths start at working directory, e.g. 
 `Datasets/iris.csv`
 * Downloading files from our git-repository

## IMPORTANT!

*Do not with absolute paths!*

Absolute paths are system-dependent and will break your code
when you move to another computer or share your code with others.

(Exeptions apply, but only very rarely - e.g. when working with system files or
HPC files installed for all users)

## Exercise 3.0b: relative paths

 * Create a subdirectory `Datasets` in your working directory
 * Move the file `iris.csv` into this subdirectory (using your file manager)
 * Read the file using a relative path

## Exercise 3.0c: using Rstudio to read files

 * In Rstudio, use the menu `File -> Import Dataset -> From Text (readr)...`
 * Select the file `iris.csv` from your `Datasets` subdirectory
 * Click on "Import"
 * What code did Rstudio generate for you?

## Exercise 3.1

Download the following files into your "Datasets" subdirectory:

 * `TB_ORD_Gambia_Sutherland_biochemicals.csv`
 * `iris.csv`
 * `meta_data_botched.xlsx` 

Which functions would you use? Go ahead, try them.
...
Reminder: One crucial difference between tibble and data frame is that `tibble[ , 1 ]` 
...
Another useful package is `skimr`, which provides a function
`skim` which provides a summary of each column:
