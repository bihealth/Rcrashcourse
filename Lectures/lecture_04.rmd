...
```{r}
df1 <- data.frame(ID=c("a", "a", "b", "c"), visit=c(1, 2, 1, 1), 
  value=rnorm(4))
df2 <- data.frame(ID=c("a", "a", "b", "c"), visit=c(1, 2, 1, 1), 
  value=rnorm(4))
merge(df1, df2)           ## not what we wanted
merge(df1, df2, by="ID")  ## not what we wanted
merge(df1, df2, by=c("ID", "visit"))
```
...
```{r}
df1$ID_visit <- paste(df1$ID, df1$visit, sep=".")
df2$ID_visit <- paste(df2$ID, df2$visit, sep=".")
merge(df1, df2, by="ID_visit")
```
